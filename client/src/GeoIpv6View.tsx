...
const useGeoIpv6Fetch = () => {
  ...
  const put = async (url, body) => {
    // Note: IPv6 uses query params for update: /api/admin/geo/ipv6/update?network=...
...
const GEO_IPV6_COLS = [
  ...
  { key: 'registered_country_geoname_id', label: 'Reg Country ID' },
  { key: 'represented_country_geoname_id', label: 'Rep Country ID' },
  { key: 'is_anonymous_proxy', label: 'Anon Proxy' },
  { key: 'is_satellite_provider', label: 'Satellite' },
  { key: 'postal_code', label: 'Postal Code' },
  { key: 'latitude', label: 'Latitude' },
  { key: 'longitude', label: 'Longitude' },
  { key: 'accuracy_radius', label: 'Accuracy' },
...
const DEFAULT_VISIBLE_COLS = {
  ...
  is_satellite_provider: false, postal_code: true, latitude: true,
...
const IconsUI = {
  ...
  power: (p) => <svg width={p.size || 16} height={p.size || 16} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...p}><path d="M18.36 6.64a9 9 0 1 1-12.73 0"></path><line x1="12" y1="2" x2="12" y2="12"></line></svg>,
...
const GeoIpv6View = ({ addToast }) => {
  const { get, post, put, patch, del } = useGeoIpv6Fetch();
  const [data, setData] = useState([]);

  const handleToggleStatus = async (item) => {
    try {
      await patch(`/api/admin/cities/${item.geoname_id}/toggle`, { isActive: !item.status });
      setData(prev => prev.map(row => row.network === item.network ? {...row, status: !item.status} : row));
    } catch (e) {
      addToast("Failed to toggle status: " + e.message, "warning");
    }
  };
  ...
  const [editingId, setEditingId] = useState(null); // Will store network string
  ...
  const [visibleCols, setVisibleCols] = useState(() => {
    const saved = localStorage.getItem('samaagum_geo_ipv6_cols');
    return saved ? JSON.parse(saved) : DEFAULT_VISIBLE_COLS;
  });

  useEffect(() => {
    localStorage.setItem('samaagum_geo_ipv6_cols', JSON.stringify(visibleCols));
  }, [visibleCols]);
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await get(`/api/admin/geo/ipv6?page=${page}&limit=${limit}&search=${encodeURIComponent(debouncedSearch)}&status=${statusFilter}&country=${encodeURIComponent(countryFilter)}&state=${encodeURIComponent(stateFilter)}&sortBy=${sortBy}&sortOrder=${sortOrder}`);
      setData(res.data);
      setTotal(res.meta.total);
      setTotalPages(res.meta.totalPages);
    } catch (e) {
      addToast("Failed to load records", "warning");
    } finally {
      setLoading(false);
    }
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingId) {
        await put(`/api/admin/geo/ipv6/update?network=${encodeURIComponent(editingId)}`, formData);
        addToast("IPv6 block updated", "success");
      } else {
        await post('/api/admin/geo/ipv6', formData);
        addToast("IPv6 block created", "success");
      }
      setModalOpen(false);
      loadData();
    } catch (err) {
  ...
  const handleDelete = async (network) => {
    if (confirm(`Are you sure you want to delete network ${network}?`)) {
      try {
        await del(`/api/admin/geo/ipv6?network=${encodeURIComponent(network)}`);
  ...
  const filteredCols = GEO_IPV6_COLS.filter(c => c.label.toLowerCase().includes(colSearch.toLowerCase()));
  ...
  return (
    <div className="data-panel" style={{display:'flex', flexDirection:'column', height:'100%', gap:'20px'}}>
      ...
      <div className="panel-header" style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
        <div>
          <h3>GeoLite IPv6 Blocks</h3>
          <p style={{ fontSize: "13px", color: "var(--ink-3)", margin: 0 }}>Manage IPv6 network blocks from GeoLite2.</p>
        </div>
      </div>
      ...
      <div style={{display:'flex', gap:'16px', flexWrap:'wrap', alignItems:'center', background:'var(--surface)', padding:'16px', borderRadius:'12px', border:'1px solid var(--border)'}}>
        <div style={{position:'relative', flex: 1, minWidth: '250px', maxWidth:'400px'}}>
          ...
          <input 
            ...
            placeholder="Search by Network, Geoname ID, Country..." 
            value={search} 
      ...
      <div className="table-wrapper" style={{ flex: 1, overflowX: 'auto', border:'1px solid var(--border)', borderRadius:'8px', position:'relative' }}>
        <table className="admin-table" style={{ whiteSpace: 'nowrap', width:'100%', fontSize:'13px' }}>
          <thead style={{position:'sticky', top:0, zIndex:5, background:'var(--surface)'}}>
            <tr>
              {GEO_IPV6_COLS.filter(c => visibleCols[c.key]).map(c => (
                <th 
                  key={c.key} 
              ...
              <th style={{position:'sticky', right:0, zIndex:10, background:'var(--surface-2)', borderLeft:'1px solid var(--border)', borderBottom:'1px solid var(--border)', width:'120px', textAlign:'center'}}>Actions</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              <tr><td colSpan={GEO_IPV6_COLS.length+1} style={{textAlign:'center', padding:'40px', color:'var(--ink-3)'}}>Loading dataset...</td></tr>
            ) : data.length === 0 ? (
              <tr><td colSpan={GEO_IPV6_COLS.length+1} style={{textAlign:'center', padding:'40px', color:'var(--ink-3)'}}>No records found.</td></tr>
            ) : data.map(item => (
              <tr key={item.network} style={{height:'44px'}}>
                {GEO_IPV6_COLS.filter(c => visibleCols[c.key]).map(c => (
                  <td key={c.key}>
                    {c.key === 'status' ? (
                      ...
                      item[c.key] !== null ? item[c.key].toString() : <span style={{color:'var(--ink-4)'}}>-</span>
                    )}
                  </td>
                ))}
                <td style={{position:'sticky', right:0, zIndex:10, background:'var(--surface)', borderLeft:'1px solid var(--border)', textAlign:'center'}}>
                  <div style={{display:'flex', justifyContent:'center', gap:'12px', alignItems:'center'}}>
                    ...
                    />
                    <button className="btn-icon" title="Edit" onClick={() => { setEditingId(item.network); setFormData(item); setModalOpen(true); }} style={{color:'var(--primary)', padding:'6px', borderRadius:'4px', background:'transparent', border:'none', cursor:'pointer'}}>
                      <IconsUI.edit />
                    </button>
                    <button className="btn-icon" title="Delete" onClick={() => handleDelete(item.network)} style={{color:'var(--error)', padding:'6px', borderRadius:'4px', background:'transparent', border:'none', cursor:'pointer'}}>
                      <IconsUI.trash />
                    </button>
                  </div>
                </td>
        ...
        <div className="modal-overlay" style={{ zIndex: 1000 }}>
          <div className="modal-card" style={{ maxWidth: '550px', maxHeight: '90vh', display:'flex', flexDirection:'column' }}>
            <div className="modal-header" style={{display:'flex', justifyContent:'space-between', alignItems:'center', padding:'20px', borderBottom:'1px solid var(--border)'}}>
              <h4 style={{margin:0}}>{editingId ? 'Edit IPv6 Block' : 'Add IPv6 Block'}</h4>
            ...
            <form onSubmit={handleSave} style={{display:'flex', flexDirection:'column', flex:1, overflow:'hidden'}}>
              <div className="modal-body" style={{padding:'20px', overflowY:'auto', display:'flex', flexDirection:'column', gap:'16px'}}>
                {GEO_IPV6_COLS.map(c => (
                  <div key={c.key} className="form-group">
                    ...
                    {c.key === 'is_anonymous_proxy' || c.key === 'is_satellite_provider' || c.key === 'is_anycast' ? (
                    ...
                    ) : c.key === 'geoname_id' || c.key === 'registered_country_geoname_id' || c.key === 'represented_country_geoname_id' || c.key === 'accuracy_radius' ? (
                      <input className="form-control" type="number" value={formData[c.key]||''} onChange={e=>setFormData({...formData, [c.key]: e.target.value?parseInt(e.target.value):null})} />
                    ) : (
                      <input className="form-control" type="text" value={formData[c.key]||''} onChange={e=>setFormData({...formData, [c.key]: e.target.value})} disabled={c.key === 'network' && !!editingId} />
                    )}
                  </div>
                ))}
              </div>
              ...
              <div className="modal-footer" style={{padding:'20px', borderTop:'1px solid var(--border)', display:'flex', gap:'12px', justifyContent:'flex-end', background:'var(--surface)'}}>
                ...
                <button type="submit" className="btn-sm btn-sm-primary">Save IPv6 Block</button>
              </div>
            </form>
          </div>
        </div>
...
window.GeoIpv6View = GeoIpv6View;
