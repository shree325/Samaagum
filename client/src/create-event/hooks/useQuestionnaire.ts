...
export function useQuestionnaire({ draft, savedDraft, editEv }: any) {
  ...
  const [formFields, setFormFields] = useState(
    draft?.formFields ?? savedDraft?.formFields ?? editEv?.venue_raw?.meta?.formFields ?? editEv?.venue?.meta?.formFields ?? [
