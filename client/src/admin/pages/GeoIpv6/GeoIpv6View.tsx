...
export const GeoIpv6View = ({ addToast }) => {
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingId) {
        await put(`/api/admin/geo/ipv6?id=${encodeURIComponent(editingId)}`, formData);
  ...
  const handleDelete = async (network) => {
    if (confirm(`Are you sure you want to delete network ${network}?`)) {
      try {
        await del(`/api/admin/geo/ipv6?id=${encodeURIComponent(network)}`);
                ...
                {GEO_IPV6_COLS.filter(c => visibleCols[c.key]).map(c => (
                  <td key={c.key}>
                    {c.key === 'status' ? (
                      ...
                      item[c.key] !== undefined && item[c.key] !== null ? String(item[c.key]) : "-"
                    )}
                  </td>
                ))}
