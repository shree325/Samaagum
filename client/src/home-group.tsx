                ...
                <div style={{ position: "relative" }}>
                  ...
                  {showShareSheet && (
                    <>
                      ...
                      <div style={{ position: "absolute", top: "50%", right: "100%", transform: "translateY(-50%)", marginRight: 8, display: "flex", gap: 8, padding: 8, background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 8, boxShadow: "var(--sh-md)", zIndex: 9999 }}>
                        {(() => {
              ...
              {tab === "invites" && (
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  ...
                  <div className="form-card" style={{ padding: 20 }}>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                      ...
                      {g.visibility === 'private' && (
                        <div>
                          <h4 style={{ margin: "0 0 8px 0", fontSize: 14 }}>Invitation Link</h4>
                          ...
                          <div style={{ display: "flex", gap: 8 }}>
                            <input className="cinput" readOnly value={`${window.location.origin}${window.location.pathname}#group=${g.id}`} onClick={e => (e.target as HTMLInputElement).select()} />
                            <button className="hbtn hbtn--soft" onClick={async () => { await copyText(`${window.location.origin}${window.location.pathname}#group=${g.id}`); alert("Copied!"); }}>Copy Link</button>
                          </div>
                        </div>
                      )}
                    </div>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {invites.filter(inv => inv.email || inv.username).map(inv => (
                      <div key={inv.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 16px", background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--r-sm)" }}>
                        <div>
                          <div style={{ fontWeight: 500 }}>{inv.email || inv.username}</div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={(e) => { if (e.target === e.currentTarget) setShowQModal(false); }}>
          <div style={{ width: '100%', maxWidth: 480, maxHeight: '90vh', overflowY: 'auto', background: 'var(--surface)', padding: 28, borderRadius: 16, display: 'flex', flexDirection: 'column', gap: 18, boxShadow: '0 8px 32px rgba(0,0,0,0.28)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {g.settings.questionnaires.map((q, i) => (
                <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  ...
                  ) : q.type === 'checkboxes' && q.options && q.options.length > 0 ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 4 }}>
                      {q.options.map((opt, optIdx) => {
            ...
            <button
              ...
              style={{ marginTop: 16 }}
              onClick={() => {
                const questions = g.settings.questionnaires;
                const requiredMissing = questions.some((q, i) => q.req && !isQuestionAnswered(q, answers[i]));
                if (requiredMissing) {
                  return alert("Please answer all required questions.");
                }
                const invalid = questions.find((q, i) => {
                  const val = answers[i];
                  if (!isQuestionAnswered(q, val)) return false;
                  return !!questionAnswerError(q, val);
                });
                if (invalid) {
                  return alert(questionAnswerError(invalid, answers[questions.indexOf(invalid)]));
                }
