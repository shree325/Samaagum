...
const useGeoIpv4Fetch = () => {
  ...
  const put = async (url, body) => {
    // Note: IPv4 uses query params for update: /api/admin/geo/ipv4/update?network=...
...
const GeoIpv4View = ({ addToast }) => {
  ...
  const [visibleCols, setVisibleCols] = useState(() => {
    const saved = localStorage.getItem('samaagum_geo_ipv4_cols');
    return saved ? JSON.parse(saved) : DEFAULT_VISIBLE_COLS;
  });

  useEffect(() => {
    localStorage.setItem('samaagum_geo_ipv4_cols', JSON.stringify(visibleCols));
  }, [visibleCols]);
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await get(`/api/admin/geo/ipv4?page=${page}&limit=${limit}&search=${encodeURIComponent(debouncedSearch)}&status=${statusFilter}&country=${encodeURIComponent(countryFilter)}&state=${encodeURIComponent(stateFilter)}&sortBy=${sortBy}&sortOrder=${sortOrder}`);
      setData(res.data);
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingId) {
        await put(`/api/admin/geo/ipv4/update?network=${encodeURIComponent(editingId)}`, formData);
        addToast("IPv4 block updated", "success");
      } else {
        await post('/api/admin/geo/ipv4', formData);
        addToast("IPv4 block created", "success");
      }
      setModalOpen(false);
      loadData();
    } catch (err) {
  ...
  const handleDelete = async (network) => {
    if (confirm(`Are you sure you want to delete network ${network}?`)) {
      try {
        await del(`/api/admin/geo/ipv4?network=${encodeURIComponent(network)}`);
  ...
  const filteredCols = GEO_IPV4_COLS.filter(c => c.label.toLowerCase().includes(colSearch.toLowerCase()));
  ...
  return (
    <div className="data-panel" style={{display:'flex', flexDirection:'column', height:'100%', gap:'20px'}}>
      ...
      <div className="panel-header" style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
        <div>
          <h3>GeoLite IPv4 Blocks</h3>
          <p style={{ fontSize: "13px", color: "var(--ink-3)", margin: 0 }}>Manage IPv4 network blocks from GeoLite2.</p>
        </div>
      </div>
      ...
      <div className="table-wrapper" style={{ flex: 1, overflowX: 'auto', border:'1px solid var(--border)', borderRadius:'8px', position:'relative' }}>
        <table className="admin-table" style={{ whiteSpace: 'nowrap', width:'100%', fontSize:'13px' }}>
          <thead style={{position:'sticky', top:0, zIndex:5, background:'var(--surface)'}}>
            <tr>
              {GEO_IPV4_COLS.filter(c => visibleCols[c.key]).map(c => (
                <th 
                  key={c.key} 
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={GEO_IPV4_COLS.length+1} style={{textAlign:'center', padding:'40px', color:'var(--ink-3)'}}>Loading dataset...</td></tr>
            ) : data.length === 0 ? (
              <tr><td colSpan={GEO_IPV4_COLS.length+1} style={{textAlign:'center', padding:'40px', color:'var(--ink-3)'}}>No records found.</td></tr>
            ) : data.map(item => (
        ...
        <div className="modal-overlay" style={{ zIndex: 1000 }}>
          <div className="modal-card" style={{ maxWidth: '550px', maxHeight: '90vh', display:'flex', flexDirection:'column' }}>
            <div className="modal-header" style={{display:'flex', justifyContent:'space-between', alignItems:'center', padding:'20px', borderBottom:'1px solid var(--border)'}}>
              <h4 style={{margin:0}}>{editingId ? 'Edit IPv4 Block' : 'Add IPv4 Block'}</h4>
            ...
            <form onSubmit={handleSave} style={{display:'flex', flexDirection:'column', flex:1, overflow:'hidden'}}>
              ...
              <div className="modal-footer" style={{padding:'20px', borderTop:'1px solid var(--border)', display:'flex', gap:'12px', justifyContent:'flex-end', background:'var(--surface)'}}>
                ...
                <button type="submit" className="btn-sm btn-sm-primary">Save IPv4 Block</button>
              </div>
            </form>
          </div>
        </div>
