...
export class EntityController {
    ...
    async createEntity(req: Request, res: Response) {
        try {
            const { tenant_id, parent_entity_id, entity_type, name, metadata } = req.body;

            let entity;
            if (entity_type === 'org') {
                entity = await entityService.createOrganization(tenant_id, name, metadata);
            } else if (entity_type === 'community') {
                entity = await entityService.createCommunity(tenant_id, parent_entity_id, name, metadata);
            } else if (entity_type === 'sub_community') {
                entity = await entityService.createSubCommunity(tenant_id, parent_entity_id, name, metadata);
            } else if (entity_type === 'group') {
                entity = await entityService.createGroup(tenant_id, parent_entity_id, name, metadata);
            } else {
