...
CREATE TABLE addons (
    ...
    -- par_row_id: Parent Row ID (links this optional addon purchase directly to its parent event).
    par_row_id                UUID         NOT NULL REFERENCES events(event_id) ON DELETE CASCADE, 
...
CREATE INDEX idx_addons_bu_id      ON addons (bu_id);
CREATE INDEX idx_addons_par_row_id ON addons (par_row_id);
